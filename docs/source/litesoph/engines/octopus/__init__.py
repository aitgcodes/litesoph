...
def pre_condition_rt_tddft_delta(status):
    ...
    try:
        if status.get('octopus.ground_state.done'):
            return (True, 'yes')
        else:
...
def pre_condition_spectrum(status):
    ...
    try:
        if status.get('octopus.rt_tddft_delta.done'):
            return (True, 'yes')
        else:
...
def pre_condition_tcm(status):
    ...
    try:
        if status.get('octopus.ground_state.done') and ['td_occup'] in status.get('octopus.rt_tddft_delta.param.TDOutput'):
            return(True, 'yes')
        else:
